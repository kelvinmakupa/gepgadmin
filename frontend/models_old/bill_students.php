...
if (mysqli_connect_errno()) {
 ...
 //   Confirmed Users
$applications = mysqli_query($obj->con, "select * from student_details where is_delete=1  and citizenship='Tanzanian' and admission_number='A/UDOM/2018/042187'");
$year_id=3;
//    Qualified Users
/*
 $applications = mysqli_query($obj->con, "SELECT pd.user_id AS  'user_id', pd.first_name, pd.middle_name, pd.last_name, gender AS  'sex',
tcu.form_four AS  'F4indexno', tcu.form_six AS  'F6Indexno', p.code, p.programme_name AS  'Programme Name', subm.phone_number AS  'phone', p.level AS  'study_level',
  '1' AS  'yos',  '2018/19' AS  'academic_year', pd.citizenship, acc.email,pd.date_of_birth as dob
FROM tcu_admission_status tcu
INNER JOIN tcu_submit_programmes subm ON subm.form_four = tcu.form_four
INNER JOIN programmes p ON p.code = subm.programme_admitted
INNER JOIN personal_details pd ON pd.user_id = subm.user_id
INNER JOIN accounts AS acc ON acc.user_id = pd.user_id
WHERE tcu.admission_status LIKE  '%Qualified%'
ORDER BY tcu.id ASC ");
*/
...
$i++;
    try{
        ...
        $reg_num_seg=explode('/',$row['reg_number']);
        $last_seg=$reg_num_seg[count($reg_num_seg)-1];

        while($fee=mysqli_fetch_assoc($fee_array)){

            $gepg_flag=false;

            if(($fee['payment_type_id']==1)||($fee['payment_type_id']==2)||($fee['payment_type_id']==33)){
                ...
                $semester='1';
                $year_=date('y');
                $control_number='DM'.$year_.$semester.$last_seg.$fee['acc_code'];
            }
