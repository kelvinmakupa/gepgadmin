...
$applications = mysqli_query($obj->con, "select * from ipabilling reg_no where reg_no is not null");            
 
/*  DEGREE       
*/
/*
$applications = mysqli_query($obj->con, "select co.name,co.check_no,fe.local_amount as bill_amount,co.year_of_study,co.program_code from continue_students co 
inner join programmes p on co.program_code=p.code
inner join tbl_department d on p.department_id=d.department_id inner join tbl_faculty f on f.faculty_id=d.faculty_id inner join tbl_college c on c.college_id=f.college_id
inner join tbl_fee_structure fe on fe.programme_id=p.program_id
where co.id>1 and fe.year_of_study=co.year_of_study and fe.academic_year_id=5 and co.year_of_study=3 and p.code='DM067'
");   

*/         
  
  /*  DIPLOMA
$applications = mysqli_query($obj->con, "select co.name,co.check_no,fe.local_amount as bill_amount from continue_students co inner join programmes p on co.program_code=p.code
inner join tbl_department d on p.department_id=d.department_id inner join tbl_faculty f on f.faculty_id=d.faculty_id inner join tbl_college c on c.college_id=f.college_id
inner join tbl_fee_structure fe on fe.programme_id=p.program_id
where co.id=1 and fe.year_of_study=co.year_of_study and fe.academic_year_id=5 and p.code like '16DP%'");      
  */    
...
$i++;
    try{         
                ...
                $currency='TZS';
                $bill_amount=10000;
			 //   $bill_amount=$row['tuition'];
				$payment_type_id=161; //to change income_code  142=ada postgraduate, 159=quality 141 undergraduate ada 144//non degree
            ...
            $bill_description='Contribution to UDOSO';       //Tuition fees-Non Degree Programmes //Tuitions Fees-Undergraduate
