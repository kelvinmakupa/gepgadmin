...
$i++;
    try{    
        ...
        while($fee=mysqli_fetch_assoc($fee_array)){     
            ...
            if(($fee['payment_type_id']==2)||($fee['payment_type_id']==33)){      
...
class OAS
{
    ...
    public static function checkData($reg_num,$payment_type_id){
            ...
            $obj=new OAS();
            $resp=mysqli_query($obj->con,"SELECT * FROM tbl_billing where bill_item_ref='{$reg_num}' and payment_type_id='{$payment_type_id}' AND year_id='4' AND is_deleted=1");
            $flag=false;
