...
$applications = mysqli_query($obj->con, "select * from graduate_reg_number where id=605");    
// $applications = mysqli_query($obj->con, "select * from phd WHERE  id>85");        
          
//$applications = mysqli_query($obj->con, "select * from student_details where is_delete=1  and citizenship='Tanzanian'");
...
$i++;
    ...
    //    $fee_array=array(['local_amount'=>'20000','acc_code'=>'45','description'=>'Quality Assurance','payment_type_id'=>'31']);
   

       $reg_num_seg=explode('/',$row['reg_no']);      
         $last_seg=$reg_num_seg[count($reg_num_seg)-2];   //second segment of the registration number
    //    $last_seg=$reg_num_seg[count($reg_num_seg)-3];   //third segment of the registration number for PHD Applicants(students)
        ...
        foreach( $fee_array as $fee){   
            ...
            if(($fee['payment_type_id']==1)||($fee['payment_type_id']==2)||($fee['payment_type_id']==33)){
                ...
                */   
                ...
                $bill_amount=$fee['local_amount'];      
                $bill_pay_opt='3';
                $semester='4';      // For graduate student the semester is 4      
                $year_=date('y');
                 $control_number='DM'.$year_.$semester.$last_seg.$fee['acc_code'];    
                //$control_number='DM'.'16'.$semester.$last_seg.$fee['acc_code'];                   
        
           /* }*/    

            $payer='3';  
           // $payer_name=str_replace("'","\'",$row['name']);
            $payer_name=str_replace("'","\'",$row['first_name']).' '.str_replace("'","\'",$row['middle_name']).' '.str_replace("'","\'",$row['last_name']);
           ...
           if(mysqli_query($obj->con,$sql)){        
            ...
            /*if($gepg_flag) { this part is for posting the bill to the gepg api to get control number from GePG
