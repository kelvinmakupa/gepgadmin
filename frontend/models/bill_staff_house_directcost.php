...
$applications = mysqli_query($obj->con, "SELECT check_no, name FROM continue_students co
INNER JOIN programmes p ON co.program_code = p.code
INNER JOIN tbl_department d ON p.department_id = d.department_id
INNER JOIN tbl_faculty f ON f.faculty_id = d.faculty_id
INNER JOIN tbl_college c ON c.college_id = f.college_id
WHERE c.college_id !=36");      
...
$i++;
    try{         
			   ...
			   // $bill_amount=$row['bill_amount'];
				$payment_type_id=113; //to change income_code  142=ada, 159=quality 
            ...
            $bill_description='Accomodation fees';       
